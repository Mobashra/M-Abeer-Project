...
with st.spinner(f"Loading {selected_group} data..."):
    # 1. Price Area Data (For Coloring)
    df = utils.load_map_stats(year, days, data_type, selected_group)
    
    # 2. Map Geometries
    geojson_areas = utils.load_geojson()
    geojson_munis = utils.load_municipality_geojson() # BONUS FILE

if not geojson_areas:
    st.error("Error: 'elspot_areas.geojson' not found.")
    st.stop()

# Ensure map label alignment
if not df.empty:
...
if show_munis:
    if geojson_munis:
        # Add Municipalities as a visual outline layer
        fig.add_trace(go.Choroplethmapbox(
            geojson=geojson_munis,
            # 'nummer' is the standard key for Basisdata from Geonorge
            ...
            marker_line_color='grey', # The border color
            marker_line_width=0.5,    # Thin lines
            showscale=False,
            hoverinfo='skip',         # Don't block clicking Price Areas
            name="Municipalities"
        ))
    else:
        st.warning("⚠️ Municipality file not found. Ensure 'Basisdata_0000_Norge_4258_Kommune_GeoJSON.geojson' is in the root folder.")
...
fig.update_layout(margin=dict(r=0, t=0, l=0, b=0), clickmode='event+select')
...
if event and "selection" in event and event["selection"]["points"]:
    ...
    if "location" in point:
        ...
        # Only update if it's a valid Price Area ID
