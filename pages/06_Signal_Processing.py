...
st.info(f"📍 **Currently Viewing:** Price Area **{global_area}**")
...
# Filter Data (Using global area since this page is simple)
df_area = df[df['price_area'] == global_area]
...
with tab1:
    ...
    with st.container():
        ...
        with c_s: seasonal = st.number_input("Seasonal Smoother (Odd)", min_value=3, value=13, step=2)
        with c_t: trend = st.number_input("Trend Smoother (Odd)", min_value=3, value=169, step=2)
...
with tab2:
    ...
    c1, c2 = st.columns(2)
    with c1: win_len = st.number_input("Window Length", min_value=10, max_value=1000, value=256, step=10)
    with c2: overlap = st.number_input("Window Overlap", min_value=0, max_value=win_len-1, value=int(win_len/2), step=10)
    ...
    if st.button("Generate Spectrogram", type="primary"):
        with st.spinner("Computing frequencies..."):
            try:
                ...
                fig = go.Figure(data=go.Heatmap(z=Sxx, x=t, y=f, colorscale="Viridis", colorbar=dict(title="Power (dB)")))
                fig.update_layout(title=f"Spectrogram: {selected_group}", yaxis_title="Freq (cycles/h)", xaxis_title="Time", yaxis_range=[0, 0.1], template="plotly_white", height=600)
                st.plotly_chart(fig, use_container_width=True)
