...
with st.container():
    ...
    with c5:
        ...
        if manual_area != current_area:
            ...
            # Only resetting coords on manual dropdown change
...
# --- BONUS: PRE-CALCULATE MUNICIPALITY CENTROIDS ---
# This creates a hidden scatter layer so clicking anywhere snaps to a valid lat/lon
centroid_data = []
if geojson_munis:
    for f in geojson_munis['features']:
        try:
            ...
            centroid_data.append({"lat": cent.y, "lon": cent.x, "name": name})
        except: continue
...
with c_map:
    ...
    if not df.empty:
        ...
        fig = px.choropleth_mapbox(
            geojson=geojson_areas, locations=["NO 1", "NO 2", "NO 3", "NO 4", "NO 5"],
            featureidkey=feature_key, mapbox_style="carto-positron",
            zoom=4.5, center={"lat": 65.0, "lon": 16.0}, opacity=0.3
        )
    ...
    if show_munis and geojson_munis:
        ...
        if muni_key:
            ...
            fig.add_trace(go.Choroplethmapbox(
                ...
                marker_line_color='rgba(20, 20, 20, 0.6)', marker_line_width=0.8,
                showscale=False, hoverinfo='skip', name="Municipalities"
            ))

    # --- LAYER 3: CLICKABLE CENTROIDS (The "Secret" Layer) ---
    # This enables clicking to get specific coordinates
    if not df_centroids.empty:
        fig.add_trace(go.Scattermapbox(
            lat=df_centroids["lat"], lon=df_centroids["lon"],
            mode='markers',
            # Invisible but clickable
            marker=go.scattermapbox.Marker(size=10, color='white', opacity=0), 
            hoverinfo='text', text=df_centroids["name"],
            name="Locations"
        ))
    ...
    if "selected_coords" in st.session_state:
        ...
        fig.add_trace(go.Scattermapbox(
            ...
            text=["📍 Selected Pin"], hoverinfo='text', name="Pin"
        ))
    ...
    if event and "selection" in event and event["selection"]["points"]:
        ...
        # 1. LOCATION CLICK (Centroid Layer)
        if "lat" in point:
        ...
        # 2. REGION CLICK (Choropleth Layer)
        # Only update region, DO NOT move the pin
...
with c_info:
    ...
    new_lat = st.number_input("Latitude", value=float(pin['lat']), format="%.4f")
    new_lon = st.number_input("Longitude", value=float(pin['lon']), format="%.4f")
    
    # Update state if manually changed
    if new_lat != pin['lat'] or new_lon != pin['lon']:
    ...
    if "elevation" in st.session_state:
        ...
        st.caption("Click map for elevation.")