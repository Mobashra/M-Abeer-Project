...
with st.container():
    ...
    with c5:
        ...
        if manual_area != st.session_state["selected_price_area"]:
            ...
            # Reset Pin to center ONLY on manual change
...
# 4. MAP VISUALIZATION
# ======================================================
# 3:1 Ratio makes the map wider vs the info panel
c_map, c_info = st.columns([3, 1])

with c_map:
    # Header aligns with Right Column Header
    ...
    if not df_clicks.empty:
        fig.add_trace(go.Scattermapbox(
            ...
            mode='markers', marker=go.scattermapbox.Marker(size=10, color='black', opacity=0),
    ...
    if event and "selection" in event and event["selection"]["points"]:
        ...
        # 1. COORDINATES (From Grid or Pin)
        if "lat" in point:
...
with c_info:
    # Header aligns with Left Column Header
    ...
    *This selects the dataset for Energy Statistics.*
    """)
    
    # 2. PIN INFO BOX (Separate)
    ...
    *This selects the location for Weather Data.*
    """)
