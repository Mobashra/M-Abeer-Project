...
with c_map:
    ...
    if not df.empty:
        fig = px.choropleth_mapbox(
            ...
            opacity=0.6, 
            # HOVER SETTINGS: Shows "NO 1" clearly
        ...
        # Manual hover template for fallback
    ...
    # --- LAYER 3: CLICK MESH (Invisible) ---
    # Opacity 0 makes it invisible, hoverinfo='skip' lets the Price Area hover shine through!
    if not df_mesh.empty:
        fig.add_trace(go.Scattermapbox(
            lat=df_mesh["lat"], lon=df_mesh["lon"],
            mode='markers',
            marker=go.scattermapbox.Marker(size=25, color='white', opacity=0), 
            hoverinfo='skip', # <--- THIS IS THE KEY FIX FOR HOVERING
            name="Click Mesh"
        ))
