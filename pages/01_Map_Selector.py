...
# ======================================================
# 2. CRITICAL: INIT STATE (Must be before sidebar render)
# ======================================================
# This prevents the KeyError by ensuring defaults exist immediately
...
# Now it is safe to render the sidebar
...
with st.container():
    ...
    with c5:
        ...
        if manual_area != st.session_state["selected_price_area"]:
            ...
            # If user manually changes dropdown, snap pin to city center
...
def generate_click_mesh():
    """Generates invisible grid points to capture clicks everywhere."""
...
def get_region_from_click(lat, lon, geo_data):
    """Calculates which Price Area polygon contains the clicked point."""
...
with c_map:
    ...
    # --- LAYER 3: CLICK MESH (Invisible Blanket) ---
    # This ensures EVERY click is registered as a Coordinate Click
    ...
    # --- LAYER 4: HIGHLIGHT BORDER (The Area Selection) ---
    ...
    # --- LAYER 5: RED PIN (The Click Location) ---
    ...
    if event and "selection" in event and event["selection"]["points"]:
        ...
        # 1. UPDATE PIN (Always happens first)
        if "lat" in point:
            click_lat = point["lat"]
            click_lon = point["lon"]
            ...
            # 3. UPDATE REGION (Calculate from Click)
            hit_id = get_region_from_click(click_lat, click_lon, geojson_areas)
            if hit_id:
                clean_id = hit_id.replace(" ", "") # "NO 1" -> "NO1"
        ...
        # Fallback for pure polygon clicks
