...
# --- 1. GLOBAL SETTINGS ---
# Ensure we have defaults if the user goes straight to this page
if "selected_price_area" not in st.session_state:
    st.session_state["selected_price_area"] = "NO1"
    
if "selected_coords" not in st.session_state:
    # Fallback to Oslo if session state is empty
...
if st.button("🚀 Train Model & Forecast"):
    ...
    # Fetch extra days before/after to handle timezone shifts safely
    ...
    # Align Weather Index - USING YOUR LOGIC + DST SAFETY
    # 1. Convert to Datetime
    df_weather['time'] = pd.to_datetime(df_weather['time'])
    
    # 2. Localize to Oslo (Handling Ambiguous Times by keeping the first occurrence)
    # Your previous code used utc=True then convert, which is safe from crashes but physically shifts time.
    # We will stick to your crash-safe method but ensure duplicates are gone.
    if df_weather['time'].dt.tz is None:
         df_weather['time'] = df_weather['time'].dt.tz_localize("Europe/Oslo", ambiguous='NaT', nonexistent='shift_forward')
    else:
         df_weather['time'] = df_weather['time'].dt.tz_convert("Europe/Oslo")
    
    # 3. Drop Duplicates (The Critical Fix for "Reindex" Error)
    df_weather = df_weather.dropna(subset=['time']) # Drop failed conversions
    df_weather = df_weather.drop_duplicates(subset=['time'])
    
    weather_series = df_weather.set_index('time').asfreq('h').interpolate()
   
    # C. Align Exogenous Data
    if exog_vars:
        # 1. Train Data Exog
